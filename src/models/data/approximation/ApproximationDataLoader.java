...
public class ApproximationDataLoader extends DataLoader {
    ...
    private Data getData(@NotNull Function function, int size) {
        Matrix inputs = getInputs(function, size);
        return new Data(inputs, getOutputs(function, inputs));
    }

    private Data getExtendedData(@NotNull Function function, int size, double extendingFactor) {
        try {
    ...
    private Matrix getInputs(@NotNull Function function, int size) {
        return getInputs(function, size, 1.0);
    }

    private Matrix getInputs(@NotNull Function function, int size, double extendingFactor) {
        return MatrixOperations.cartesianProduct(function.getVariableRanges().stream()
                .map(vr -> vr.getExtendedRange(size, extendingFactor)).collect(Collectors.toList()));
    }

    private Matrix getOutputs(@NotNull Function function, @NotNull Matrix inputs) {
