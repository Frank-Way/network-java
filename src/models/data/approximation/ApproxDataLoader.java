...
public class ApproxDataLoader extends DataLoader {
    ...
    protected Data getTrainData(LoadParameters parameters) {
        if (parameters instanceof ApproxLoadParameters) {
            ...
            return getExtendedData(approxLoadParameters.getFunction(), approxLoadParameters.getSizes(),
                    approxLoadParameters.getExtendingFactors());
        }
     ...
     * @return         выборка
     */
    private Data getData(Function function, int[] sizes) {
        Matrix inputs = getInputs(function, sizes);
     ...
     */
    private Data getExtendedData(Function function, int[] sizes, double[] extendingFactors) {
        try {  // оборачивание в try-catch, так как в результате расширения может быть получен диапазон,
               // где функция не определена
            final Matrix inputs = getInputs(function, sizes, extendingFactors);
            return new Data(inputs, getOutputs(function, inputs));
        } catch (Exception e) {
            ...
            final Matrix inputs = getInputs(function, sizes);
            return new Data(inputs, getOutputs(function, inputs));
        }
    }
     ...
     */
    private Matrix getInputs(Function function, int[] sizes) {
     ...
     * @return                 входы для выборки
     */
    private Matrix getInputs(Function function, int[] sizes, double[] extendingFactors) {
